...
| 2026-09-24 | v1.4 | **Penyederhanaan menu (sidebar)**: menu dikelompokkan jadi `Utama` (Dashboard, Kasir, Digital), `Stok` (Produk, Kategori, Supplier, Pembelian), `Pelanggan` (mandiri), `Laporan` (Laporan, Penjualan, Riwayat Harga), `Produksi` (BOM, Produksi), dan `Pengaturan` (Pengaturan, Pengguna, Profil) sebagai accordion; grup otomatis terbuka saat submenu aktif; peran pengguna disaring per grup. | `web/src/app/(app)/components/Sidebar.tsx` |