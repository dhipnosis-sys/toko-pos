...
-- -----------------------------------------------------------------------------
-- digital_types — extensible catalog of digital sale types (token PLN, pascabayar, cashout, ...)
-- -----------------------------------------------------------------------------
...
create trigger digital_types_updated_at
  before update on public.digital_types
  for each row execute function public.set_updated_at();
...
-- -----------------------------------------------------------------------------
-- digital_sales — recorded digital transactions (token, bill payment, cashout, ...)
-- -----------------------------------------------------------------------------
...
create trigger digital_sales_updated_at
  before update on public.digital_sales
  for each row execute function public.set_updated_at();

-- -----------------------------------------------------------------------------
-- digital_balance_movements — ledger of modal top-ups (-) / deductions (+)
-- -----------------------------------------------------------------------------
...
-- digital_types (reads for all; writes only via RPC) --------------------
...
-- digital_sales (reads for all; writes only via RPC) --------------------
...
-- digital_balance_movements (reads for all; writes only via RPC) --------
...
-- -----------------------------------------------------------------------------
-- digital: toggle type active (owner)
-- -----------------------------------------------------------------------------
